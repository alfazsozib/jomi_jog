...
const fmtDate = (key) =>
  new Date(key + "T00:00:00").toLocaleDateString("bn-BD", {
    weekday: "long",
    year: "numeric",
    month: "long",
    day: "numeric",
...
const NotificationPanel = ({
  ...
  <>
    ...
    >
      ...
      >
        <span style={{ fontWeight: 800, fontSize: 15, color: C.dark }}>
          🔔 নোটিফিকেশন
        </span>
            ...
            >
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p
                    style={{ margin: "3px 0 0", fontSize: 11, color: C.muted }}
                  >
                    {n.createdAt
      ...
      >
        ...
        >
          <div>
            ...
            >
              {existing ? "বুকিং নোট সম্পাদনা" : "বুকিং নোট যোগ করুন"}
            </h3>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            <label style={lbl}>ক্লায়েন্টের নাম</label>
            <input
              name="clientName"
          ...
          <div>
            <label style={lbl}>মোবাইল নম্বর</label>
            <input
              name="clientMobile"
          ...
          <div>
            <label style={lbl}>জরিপের স্থান</label>
            <input
              name="location"
              value={form.location}
              onChange={handle}
...
const Calendar = ({ bookedDates, noteEvents, onDayClick }) => {
  ...
  const daysInMonth = getDaysInMonth(cur.year, cur.month),
    firstDay = getFirstDay(cur.year, cur.month);
  const cells = [];
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      >
        <button style={S.navBtn} onClick={prev}>
          ‹
        </button>
        <span style={{ fontWeight: 800, fontSize: 17, color: C.dark }}>
            ...
            <div
              ...
              title={
                isPast
                  ? "অতীত তারিখ"
                  : isNote
                    ? `বুকিং: ${noteEvents[key]?.clientName || "নোট"}`
                    : isOther
                      ? "ব্লক করা"
                      : "পাওয়া যাচ্ছে"
              }
      ...
      <div style={S.legend}>
        <span style={S.li}>
          <span style={{ ...S.ld, background: C.primary }} /> পাওয়া যাচ্ছে
        </span>
        <span style={S.li}>
          <span style={{ ...S.ld, background: C.amber }} /> নোট বুকিং
        </span>
        <span style={S.li}>
          <span style={{ ...S.ld, background: C.red }} /> ব্লক করা
        </span>
        <span style={S.li}>
          <span style={{ ...S.ld, background: "#d1d5db" }} /> অতীত
        </span>
      </div>
    </div>
  );
...
const SurveyorDashboard = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      console.error("fetchBookings:", err.response?.status, err.response?.data);
    }
  };
  ...
  const fetchNotifications = async () => {
    try {
      const { data } = await axios.get(
        `https://jomijog.com/api/bookings/notifications/${user._id}`,
      );
  ...
  const markAllRead = async () => {
    try {
      await axios.put(
        `https://jomijog.com/api/bookings/notifications/${user._id}/read`,
      );
  ...
  const handleDayClick = (key) => {
    if (bookedDates.includes(key)) {
      if (window.confirm(`${fmtDate(key)} — এই তারিখের ব্লক সরাতে চান?`)) {
        ...
        const newNotes = { ...noteEvents };
        delete newNotes[key];
        setBookedDates(newDates);
        setNoteEvents(newNotes);
  ...
  const handleChooseNote = () => {
    setNoteDate(choiceDate);
    setEditingNote(null);
    setChoiceDate(null);
  };
  const handleChooseOther = async () => {
    const key = choiceDate;
    setChoiceDate(null);
    const newDates = [...bookedDates, key];
    setBookedDates(newDates);
    await persistToServer(newDates, noteEvents);
  };

  const handleSaveNote = async (key, form) => {
    const newDates = bookedDates.includes(key)
      ? bookedDates
      : [...bookedDates, key];
  ...
  const handleCompleteBooking = async (booking) => {
    ...
    if (
      !window.confirm(
        "এই বুকিং সম্পন্ন হিসেবে চিহ্নিত করবেন? তারিখ আবার উপলব্ধ হবে।",
      )
    )
      return;
  ...
  const noteList = Object.entries(noteEvents).sort(([a], [b]) =>
    a.localeCompare(b),
  );
  ...
  return (
    <div style={S.page}>
      ...
      {choiceDate && (
        <ChoiceDialog
          date={choiceDate}
          onNote={handleChooseNote}
          onOther={handleChooseOther}
        ...
        {saveMsg && (
          <div
            style={{
              ...
              borderRadius: 10,
              textAlign: "center",
              background: saveMsg.startsWith("✓") ? C.primaryLight : C.redLight,
              color: saveMsg.startsWith("✓") ? C.primaryDark : C.red,
              fontWeight: 700,
              fontSize: 14,
            }}
          >
        ...
        <div style={S.tabs}>
          {[
            ...
            <button
              key={tab.id}
              style={
                activeTab === tab.id ? { ...S.tab, ...S.activeTab } : S.tab
              }
        ...
        {activeTab === "calendar" && (
          <div style={{ display: "flex", justifyContent: "center" }}>
            <div style={S.calCard}>
              ...
              >
                যেকোনো ভবিষ্যত তারিখে ক্লিক করুন। <strong>নোট</strong> বেছে নিলে
                বুকিং তথ্য যোগ করতে পারবেন, আর <strong>অন্যান্য</strong> বেছে
                নিলে ব্যক্তিগত কারণে ব্লক হবে। চিহ্নিত তারিখে আবার ক্লিক করলে
                সরানো যাবে।
              </p>
              <Calendar
                ...
                return (
                  <div key={key} style={S.eventCard}>
                    ...
                    <div style={S.eventBody}>
                      {ev.clientName && (
                        <div style={S.evRow}>
                          <span style={S.evLabel}>👤 ক্লায়েন্ট</span>
                          <span style={S.evVal}>{String(ev.clientName)}</span>
                        </div>
                      )}
                      {ev.clientMobile && (
                        <div style={S.evRow}>
                          <span style={S.evLabel}>📱 মোবাইল</span>
                          <span style={S.evVal}>{String(ev.clientMobile)}</span>
                        </div>
                      )}
                      {ev.location && (
                        <div style={S.evRow}>
                          <span style={S.evLabel}>📍 স্থান</span>
                          <span style={S.evVal}>{String(ev.location)}</span>
                        </div>
                      )}
                      {ev.note && (
                        <div
                        ...
                        >
                          <span style={S.evLabel}>🗒 নোট</span>
                          <span
                            style={{ ...S.evVal, color: C.muted, fontSize: 13 }}
                          >
                            {String(ev.note)}
                          </span>
                        </div>
                  ...
                  >
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (
                          !window.confirm(
                            `তারিখ ${fmtDateShort(b.date)} ক্যালেন্ডারে ব্লক করবেন?`,
                          )
                        )
                          return;
